...
from src.model.reranker import rerank_with_bgi
...
def reranked_topk(query: str, candidate_k: int = 50, k: int = 10) -> list[str]:
    ...
    return [d for d, _ in rerank_with_bgi(query, candidates, corpus_by_id, k=k)]
...
if __name__ == "__main__":
    print(f"\nNDCG@10  on FiQA ({len(sample)} sampled test queries)")
    print("-" * 42)
